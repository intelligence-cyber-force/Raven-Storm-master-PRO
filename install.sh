...
# Schedule the uninstallation script to run after 24 hours
echo "#!/bin/bash
# Uninstall script to remove the tool and its files

# Add paths to the files and directories to be deleted
INSTALL_DIR="/path/to/installation"
SCRIPT_DIR="/path/to/installation/Raven-Storm-master"

# Remove the installed files and directories
rm -rf $INSTALL_DIR
rm -rf $SCRIPT_DIR

# Remove this script itself
rm -- "$0"
" > /tmp/uninstall_tool.sh
chmod +x /tmp/uninstall_tool.sh
(crontab -l 2>/dev/null; echo "$(date -d '24 hours' '+%M %H %d %m *') /tmp/uninstall_tool.sh") | crontab -
